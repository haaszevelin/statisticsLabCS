...
Next, click [Analyze]{style="color:blue"}, then [Correlate]{style="color:blue"}, then [Bivariate]{style="color:blue"}:

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/3.4.12.png')
```

The next window will ask you to select variables to correlate. Since we have two (`x` and `y`) move them both from the left-hand field to the right-hand field using the arrow. Notice that in this window, [Pearson]{style="color:blue"} is selected. This is the default setting (and the one we want), but notice there are other ways to calculate the correlation between variables. We will stick with Pearson's correlation coefficient for this course.

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/3.4.13.png')
```

Now, click [OK]{style="color:blue"}.
...
From here, go to [Analyze]{style="color:blue"}, then [Correlate]{style="color:blue"}, then [Bivariate]{style="color:blue"}:
...
Click [OK]{style="color:blue"}. SPSS will produce an output table that contains correlations for every pairing of our three variables, along with the correlations of each variable with itself.
...
Let's find the correlation coefficient between these variables first. Go to [Analyze]{style="color:blue"}, then [Correlate]{style="color:blue"}, then [Bivariate]{style="color:blue"}:
...
Let's continue to create the scatterplot for this data. Go to [Graphs]{style="color:blue"}, then [Legacy Dialogs]{style="color:blue"}, then [Scatter...]{style="color:blue"}

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/3.4.22.png')
```

In the next window, choose [Simple]{style="color:blue"}, then [Define]{style="color:blue"}:
...
Click [OK]{style="color:blue"}. SPSS will produce a scatterplot of your data, as follows:
...
In this window, find the button at the top that reads [Fit Line at Total]{style="color:blue"} when you hover your mouse over it. Below, I have highlighted it for clarity:

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/3.4.27.png')
```

Press this button and you will see a new menu. Make sure [Linear]{style="color:blue"} is selected and click [Apply]{style="color:blue"}.

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/3.4.28.png')
```

Next, exit from the Chart Editor. This means you will hit the [X]{style="color:blue"} in the corner of the window. You will find that the graph in your output window has now updated and has a line drawn on it.
...
In order to split the data, we go to the top menu and choose [Data]{style="color:blue"}, then [Split file...]{style="color:blue"}

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/3.4.30.png')
```

In the next window, you must select [Organize output by groups]{style="color:blue"} and then specify which variable will be used to split the data. Select `year` and move it to the right-hand field using the arrow.

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/3.4.31.png')
```

Click [OK]{style="color:blue"}. Notice that this will cause the output window to produce some next indicating that you have split your file. You can ignore this and go back to your data window.

From here, any analysis you choose to do will be done separately for each year's worth of data. Let's calculate the correlation coefficient, as usual. Click [Analyze]{style="color:blue"}, then [Correlate]{style="color:blue"}, then [Bivariate]{style="color:blue"}:
...
Click [OK]{style="color:blue"}. Notice that in the output window you will see a bunch of correlation tables (13 of them to be exact); one for each year. Scroll down and find the table with the heading "year = 2017". That's the table we need in order to answer our question:
...
![](images/variablepair.png)

or

![](images/variablepair2-01.png)
...
![](images/vpposnegeff3regrline.png)
