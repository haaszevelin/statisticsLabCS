...
<iframe width="560" height="315" src="https://www.youtube.com/embed/voMIi1fpe5c" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen>
...
Let's first get an idea about life expectancy in general. We want to plot a histogram with life expectancy on the x-axis and frequency on the y-axis. Choose [Graphs]{style="color:blue"} in the top menu, then [Legacy Dialogs]{style="color:blue"}. From here, remember we want a histogram, not a bar graph, so let's select [Histogram...]{style="color:blue"}.

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/1.4.18.png')
```

The window that appears contains every variable in your spreadsheet listed on the left-hand side. We can choose one variable at a time to plot. Let's scroll all the way down the list and choose `2017 [v219]`. This is the variable containing life expectancies for the year 2017. Using the arrow, move that variable into the field labeled "Variable:", then click [OK]{style="color:blue"}.
...
To create the scatterplot, let's go to [Graphs]{style="color:blue"} in the menu toolbar, then [Legacy Dialogs]{style="color:blue"}, then [Scatter]{style="color:blue"}.

```{r , echo=FALSE,dev='png'}
knitr::include_graphics('img/1.4.21.png')
```

You will choose [Simple]{style="color:blue"} scatter, then click [Define]{style="color:blue"}.
...
Then click [OK]{style="color:blue"}. SPSS will produce output containing a scatterplot. What relationship do you notice? What happens to life expectancy the more individuals earn, on average?
...
![](images/preferences-01.png)

![](images/detailederror-01.png)
...
Do you have any guesses? Is it Manhattan, Brooklyn, Queen’s, or Staten Island? We might have a guess, but among this large pile of data, it probably will be wrong, and also, guessing is not scientific. We can find the answer by plotting the data, using a bar plot. We just need to count how many film permits are requested in each borough, and then make different bars to represent the counts.

#### Running your first analysis (Explore variable)
...
![](images/explorevariable1-01.png)

![](images/explorevariable2-01.png)
...
![](images/explorevariables-01.png)
...
![](images/1.4.4.png) As you see with long variable names the titles are pretty hard to see, because of this reason we recommend you to name your data with shorter names. We would like to add, that this problem is noticed and we are working on it to solve the readability of titles with long names.

![](images/1.4.5.png) We can see that episodic series are the most common form of all of the subcategories, and the frequencies table shows us that it means 15416 permits, which is 30.4% of the total.

### Categories by different Boroughs (Comparing groups)
...
![](images/comparegroups.png)

![](images/comparegroups2.png)
...
![](images/charttheme.png)

Another method is to change the “Image format” to “SVG” and choose a program, in which you can make changes to the graph, so you can edit it (e.g. in LibreOffice). Note that you cannot manually change the plots in CogStat. (for more: <https://doc.cogstat.org/Displaying-the-data-and-results-graphically>)

### Life Expectancy Histogram - How long do people live worldwide according to this dataset?

Gapminder (<https://www.gapminder.org/>) is an organisation that collects some really interesting worldwide data. They also make visualization tools for looking at the data. There are many examples, and the tools are built right into their website so you can try out Gapminder Tools (<https://www.gapminder.org/tools/#$chart-type=bubbles&url=v2>) as well.
...
![](images/variablepair1.png)

or

![](images/variablepair2.png)
...
![](images/variablepairplot.png)

### Using the results pane in CogStat
